...
ax.set_title("Matriz de Confusión - SVC")
...
ax.set_ylim(0, 1.05)
ax.set_ylabel("Score")
ax.set_title("Métricas por clase y macro (SVC)")
ax.legend()
plt.tight_layout()
plt.show()
...
# 8) Fronteras de decisión en PCA 2D (solo visualización)
pca_viz = PCA(n_components=2, random_state=42)
X_train_pca = pca_viz.fit_transform(X_train)
X_test_pca  = pca_viz.transform(X_test)

clf_viz = SVC(kernel="rbf", C=1.0, gamma="scale", random_state=42)
...
ax.set_title("Process (PCA 2D) — Regiones de decisión (SVC)")
ax.set_xlabel("PC1")
ax.set_ylabel("PC2")
ax.legend(loc="best")
plt.tight_layout()
...
# 9) Dispersión por pares de atributos
fig = plt.figure(figsize=(7,5))
ax = fig.add_subplot(111)
for cls in np.unique(y):
