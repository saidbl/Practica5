...
ax.set_ylim(0, 1.05)
ax.set_ylabel("Score")
ax.set_title("Métricas por clase y macro (Gradient Boosting)")
ax.legend()
plt.tight_layout()
plt.show()
...
gb_viz.fit(X_train_pca, y_train)
...
Z = gb_viz.predict(np.c_[xx.ravel(), yy.ravel()]).reshape(xx.shape)
...
correct = y_test == gb.predict(X_test)
...
ax.set_title("Process (PCA 2D) — Regiones de decisión (Gradient Boosting)")
ax.set_xlabel("PC1")
ax.set_ylabel("PC2")
ax.legend(loc="best")
plt.tight_layout()
