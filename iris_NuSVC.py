...
# 3) NuSVC 
clf = NuSVC(nu=0.3, kernel='rbf', gamma='scale', probability=True, random_state=42)
...
ax.set_title("Matriz de Confusión - NuSVC")
...
ax.set_ylim(0, 1.05)
ax.set_ylabel("Score")
ax.set_title("Métricas por clase y macro (NuSVC)")
ax.legend()
plt.tight_layout()
plt.show()

# 7) PCA 2D — etiquetas reales
...
# 8) Fronteras de decisión (solo visualización)
pca_viz = PCA(n_components=2, random_state=42)
X_train_pca = pca_viz.fit_transform(X_train)
X_test_pca  = pca_viz.transform(X_test)

clf_viz = NuSVC(nu=0.3, kernel='rbf', gamma='scale', probability=True, random_state=42)
...
Z = clf_viz.predict(np.c_[xx.ravel(), yy.ravel()]).reshape(xx.shape)
...
ax.set_title("Process (PCA 2D) — Regiones de decisión (NuSVC)")
ax.set_xlabel("PC1")
ax.set_ylabel("PC2")
ax.legend(loc="best")
plt.tight_layout()
...
# 9) Dispersión por pares de características
fig = plt.figure(figsize=(7,5))
ax = fig.add_subplot(111)
for cls in np.unique(y):
