...
# 3) MLPClassifier (modelo principal en 4D)
...
ax.set_title("Matriz de Confusión - MLPClassifier")
...
ax.set_ylim(0, 1.05)
ax.set_ylabel("Score")
ax.set_title("Métricas por clase y macro (MLPClassifier)")
ax.legend()
plt.tight_layout()
plt.show()

# 7) Graph pre-process: PCA 2D con etiquetas reales
...
# 8) Graph process data: regiones de decisión
pca_viz = PCA(n_components=2, random_state=42)
X_train_pca = pca_viz.fit_transform(X_train)
X_test_pca  = pca_viz.transform(X_test)

clf_viz = MLPClassifier(hidden_layer_sizes=(100,), max_iter=1000, random_state=42)
...
ax.set_title("Process (PCA 2D) — Regiones de decisión (MLPClassifier)")
ax.set_xlabel("PC1")
ax.set_ylabel("PC2")
ax.legend(loc="best")
plt.tight_layout()
...
# 9) Dispersión en pares de features
fig = plt.figure(figsize=(7,5))
ax = fig.add_subplot(111)
for cls in np.unique(y):
