...
# 3) AdaBoost (stump como base) con compatibilidad de versión
base = DecisionTreeClassifier(max_depth=1, random_state=42)
try:
    ada = AdaBoostClassifier(estimator=base, n_estimators=50, random_state=42)   
except TypeError:
    ada = AdaBoostClassifier(base_estimator=base, n_estimators=50, random_state=42)  

ada.fit(X_train, y_train)
y_pred = ada.predict(X_test)

# 4) Métricas: Accuracy, Precision, Recall, F1
...
# 6) Gráfica de Métricas (Precision, Recall, F1) por clase + macro
...
ax.set_ylim(0, 1.05)
ax.set_ylabel("Score")
ax.set_title("Métricas por clase y macro (AdaBoost)")
ax.legend()
plt.tight_layout()
plt.show()
...
# 8) Graph process data: fronteras en PCA 2D (visualización)
pca_viz = PCA(n_components=2, random_state=42)
X_train_pca = pca_viz.fit_transform(X_train)
X_test_pca  = pca_viz.transform(X_test)

try:
    viz_clf = AdaBoostClassifier(estimator=DecisionTreeClassifier(max_depth=1, random_state=42),
                                 n_estimators=50, random_state=42)
except TypeError:
    viz_clf = AdaBoostClassifier(base_estimator=DecisionTreeClassifier(max_depth=1, random_state=42),
                                 n_estimators=50, random_state=42)
viz_clf.fit(X_train_pca, y_train)
...
# Train (suave) y Test (aciertos/errores) — etiquetas reales del modelo 4D
ax.scatter(X_train_pca[:, 0], X_train_pca[:, 1], c=y_train, s=15, alpha=0.6, label="Train")
correct = y_test == ada.predict(X_test)
...
ax.set_title("Process (PCA 2D) — Regiones de decisión (solo visualización)")
ax.set_xlabel("PC1")
ax.set_ylabel("PC2")
ax.legend(loc="best")
plt.tight_layout()
