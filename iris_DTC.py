...
print("=== DecisionTreeClassifier (Iris) ===")
...
ax.set_ylim(0, 1.05)
ax.set_ylabel("Score")
ax.set_title("Métricas por clase y macro (Decision Tree)")
ax.legend()
plt.tight_layout()
plt.show()
...
# 8) Graph process data: fronteras en PCA 2D (solo visualización)
...
viz_tree.fit(X_train_pca, y_train)
h = 0.02
x_min, x_max = X_train_pca[:, 0].min() - 1, X_train_pca[:, 0].max() + 1
y_min, y_max = X_train_pca[:, 1].min() - 1, X_train_pca[:, 1].max() + 1
xx, yy = np.meshgrid(np.arange(x_min, x_max, h),
                     np.arange(y_min, y_max, h))
Z = viz_tree.predict(np.c_[xx.ravel(), yy.ravel()]).reshape(xx.shape)
fig = plt.figure(figsize=(8,6))
ax = fig.add_subplot(111)
ax.contourf(xx, yy, Z, alpha=0.25)
ax.scatter(X_train_pca[:, 0], X_train_pca[:, 1], c=y_train, s=15, alpha=0.6, label="Train")
correct = y_test == tree.predict(X_test)
...
ax.set_title("Process (PCA 2D) — Regiones de decisión (Decision Tree)")
ax.set_xlabel("PC1")
ax.set_ylabel("PC2")
ax.legend(loc="best")
plt.tight_layout()
